...
-- To allow the function above to be used with RollIndex instead of RollSorted.
fillRowIndexed :: Int -> RollIndex -> Int
fillRowIndexed row roll = fillRow row (toRollSorted roll)
...
type ScoreTableEntry = (RollIndex,Int,Int)

scoreTable :: [ScoreTableEntry]
scoreTable = [ (roll,row,fillRowIndexed row roll) | roll <- setRolls, row <- [1..13] ]
