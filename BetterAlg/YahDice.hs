...
Compare to the 'Initial' version. The big change that the reroll probability
table is broken into two tables. 

The idea is that different roll/reroll choice combinations can lead to the 
same set of outcomes and probabilties. Let [abxxxx] represent a roll with
the reroll choice being indicated by the variables used: using the letters
abcde means that those dice of the initial roll are not rerolled, and x is
used for dice that will be rerolled. The different values that might appear 
at each x are irrelevant since that die will be rerolled. You get the same 
set of reroll outcomes (and probabilities) iff the the dice that are not 
being rerolled are the same. Thus, [abxxx], [axbxx], [axxbx], etc., all lead 
to the same thing. 

There are 4,368 possible ways to reroll; that is, there are 252 intial rolls,
each of which has some number of ways one might reroll (from 6 to 32), and 
the sum of these ways to reroll over all 252 initial rolls is 4,368. Each 
choice for rerolling leads to some number of outcomes (rerolling 5d6 has 
252 outcomes, rerolling 4d6 has 126 outcomes, etc.). Sum up these values over 
each of the 4,368 reroll choices, and there are 254,100 possible "paths."

However, 254,100 is a significant overestimate for the distinct cases when 
considering the amount of data that must be held/examined when all initial 
rolls are considered together. For example, it doesn't matter what the 
initial roll is if all five dice are rerolled. Part of the 254,100 total
includes 252 initial rolls, followed by rerolling all five, followed by the
252 outcomes. So, 252x252 = 63,504 cases of the 254,100 could be covered by 
one set of 252 values. Similarly, if three dice are rerolled, then all of
these cases lead to the same set of outcomes: [12xxx], [1x2xx], [1xx2s], etc.

The strategy that's most efficient is to break the table up into two. One 
table looks like
[(initial roll,[(reroll choice,[outcome table index])])]
and another table looks like
[[(reroll outcome,outcome probability)]]
That is, instead of putting the actual data into the original table,
point to it from another table. In something like C, you would use actual
pointers (and the fact that you are using this trick would be less visible);
in Haskell we have to make our own pointers by using an integer index.
Another thing you could do in C, with mutable variables, is more easily
"memoize" whether a calculation has been done with something like
'if result == null then do_the_calculation'.

The savings as a result of this change is roughly ten-fold, which is larger
than one might expect. Part of the reason for the large savings is due to the
precise circumstances in which redundance arises. Some cases with little 
redundance are those of five distinct die values and none of them being
rerolled. There is only one "outcome" in that scenario: the original roll. 
There's no savings in these cases, but there was nothing much to save to begin
with: a calculation involving only a single roll. At the other extreme, when 
all five dice are rerolled, it doesn't matter what the original face values
were, so all 252 of these cases are the same, and this case involves (or 
these cases involve) a larger calculation.
...
toRollChoiceIndex :: RollSorted -> RerollChoice -> RollAndChoiceIndex
toRollChoiceIndex roll cs = theIndex (notFaces roll cs) (5 - (sum $ map fromEnum cs))
  where
...
indexAndRollToChoice :: RollAndChoiceIndex -> RollSorted -> RerollChoice
indexAndRollToChoice i roll = theChoice
  where
    ...
    theChoice = reverse $ go [] roll subRoll
      where
        go cs [] _ = cs
        go cs (r:rs) sub@(s:ss) 
          | r `elem` sub = go (False:cs) rs ss
          | otherwise = go (True:cs) rs sub
        go cs (r:rs) [] = go (True:cs) rs []
...
allRROutcomes :: [[(RollSorted,Double)]]
allRROutcomes = answer
  where
    listActual = (map indexToRollChoice rangeRollAndChoiceIndex)
      :: [(RollSorted,RerollChoice)]
    -- listOut = (map (\(rs,c) -> rrProbTableII rs c) listActual)
    answer = (map (\(rs,c) -> rrProbTableII rs c) listActual)
      :: [[(RollSorted,Double)]]
