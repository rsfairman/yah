...
This version avoids some redundant calculations by moving certain things
outside. See YahDice.hs.
...
import YahDice(RollSorted,RerollChoice,
            setRolls,theRollProbs,allRROutcomes,
            getRRChoices,toRollChoiceIndex,rangeRollAndChoiceIndex,indexAndRollToChoice)
    ...
    entry n = go (getFinalEVNEW (entry (n-1)) )
      where
        go prevFinal = do
          ...
          -- RowOptions = [(RollSorted,RowChoice,EV)]
          ...
          let rrOuts = allRROutcomes :: [[(RollSorted,Double)]]
          ...
          -- EV of that roll.
          -- 
          -- So, this transforms rrOuts from type [(RollSorted,Probability]
          -- to [(EV,Probability)], with EV relative to rrDone.
          let lookupEV aRoll = (\(_,_,ev) -> ev) $ fromJust $ find (\(ra,_,_) -> ra == aRoll) rrDone
          ...
          -- And the next set of choices and EVs for the player: result is
          -- RerollOptions = [(RollSorted,RerollChoice,EV)]
          let oneRRLeft = getRRChoicesNEW choiceEVs
          
          -- Same thing again:
          let lookup2 aRoll = (\(_,_,ev) -> ev) $ fromJust $ find (\(ra,_,_) -> ra == aRoll) oneRRLeft
          let rr1EVs = ((map.map) (\(theRoll,theProb) -> (lookup2 theRoll,theProb)) rrOuts)
                :: [[(Double,Double)]]
          let choices2 = ( map weightedSum rr1EVs ) :: [Double]
          let twoRRLeft = getRRChoicesNEW choices2
...
getRRChoicesNEW :: [Double] -> RerollOptions
getRRChoicesNEW choiceEVs = do
    r <- setRolls
    ...
    return (r,choice,ev) -- (RollSorted,RerollChoice,EV)
