...
This version makes a grab-bag of little tweaks to speed things up.
The most obvious thing to outside code is that RerollChoice is now
an Int instead of a list. Internal to this module, [Bool] is still
used, because it's so much easier,
...
-- No point in vectorizing this since it's used so little.
allNd6 :: Int -> [Roll]
allNd6 0 = [[]]
...
-- It is still based on RollSorted, although it turns out that this is used
-- so little that vectorizing it probably doesn't matter.
setNd6V :: Vb.Vector (Vb.Vector RollSorted)
setNd6V = Vb.fromList [Vb.fromList (setNd6 n) | n <-[0..5]]
...
-- Based on profiling, seems to be no point in trying to speed this up.
probsNd6 :: Int -> [(RollSorted,Double)]
probsNd6 n = [ (r,fromIntegral(doCount r) / fromIntegral(length allSorted)) | r <- Vb.toList (setNd6V Vb.! n)]
  where
...
--
-- The implementation is stil slow, but the use is hidden behind a table
-- that's build only once, so it doesn't matter very much.
toRollSubIndex :: Int-> RollSorted -> Int
toRollSubIndex n r = fromJust $ elemIndex r $ Vb.toList (setNd6V Vb.! n)
...
setRollsSorted = Vb.toList (setNd6V Vb.! 5)
...
toRSV :: Vb.Vector (RollSorted)
toRSV = Vb.fromList [ (setRollsSorted !! i) | i <- [0..251] ]

toRollSorted :: RollIndex -> RollSorted
toRollSorted i = toRSV Vb.! i
...
-- And for interfacing to the outside, this is used. It's the same as
-- the above, but packed to a value in the range [0..31].
type IntRerollChoice = Int

-- Convert a [Bool] to a packed int.
toIntRerollChoice :: RerollChoice -> IntRerollChoice 
toIntRerollChoice c = sum $ map (\(x,y) -> x*y) $ zip (map fromEnum c) [1,2,4,8,16]

-- Unpack an int to a [Bool]
fromIntRerollChoice :: IntRerollChoice -> RerollChoice
fromIntRerollChoice i = map (testBit i) [0,1,2,3,4]
...
rerollChoices :: Vb.Vector ([IntRerollChoice])
rerollChoices = Vb.fromList [ map toIntRerollChoice (choices r) | r <- [0..251] ]
  where
    choices r = snd $ recurDiffRRs (toRollSorted r) allRR ([],[])

getRRChoices :: RollIndex -> [IntRerollChoice]
getRRChoices r = rerollChoices Vb.! r
...
toRollChoiceIndexInternal :: RollIndex -> RerollChoice -> RollAndChoiceIndex
toRollChoiceIndexInternal roll cs = theIndex (notFaces (toRollSorted roll) cs) (5 - (sum $ map fromEnum cs))
  where
...
-- The above is used a lot. Vectorize it.
-- "toRCIndexV Vb.! n" is the sub-table for the n-th RollIndex, and
-- "(toRCIndexV Vb.! n) Vu.! c" is the further indexing by the 
-- IntRerollChoice value, many of which are blank since they don't
-- make sense -- but the table faster to access.
toRCIndexV :: Vb.Vector (Vu.Vector RollAndChoiceIndex)
toRCIndexV = Vb.fromList [
  Vu.fromList [ toRollChoiceIndexInternal r (fromIntRerollChoice c) | c <- [0..31] ] | r <- [0..251] ]

-- Convenient means of access
-- The INLINE seems to do nothing for speed.
-- {-# INLINE toRollChoiceIndex #-}
toRollChoiceIndex :: RollIndex -> IntRerollChoice -> RollAndChoiceIndex
toRollChoiceIndex r c = (toRCIndexV Vb.! r) Vu.! c
...
indexToRollChoice i = (roll,choice)
  where
    ...
    -- subRoll = (setNd6 bin) !! subIndex
    subRoll = (setNd6V Vb.! bin) Vb.! subIndex
...
indexAndRollToChoiceInternal :: RollAndChoiceIndex -> RollIndex -> RerollChoice
indexAndRollToChoiceInternal i roll = theChoice
  where
    -- The number of dice not rerolled.
    bin
...
-- iaRtCV ! rc ! ri
-- is the same as
-- indexAndRollToChoiceInternal rc ri
iaRtCV :: Vb.Vector (Vu.Vector Int)
iaRtCV = Vb.fromList [ Vu.fromList [toIntRerollChoice(indexAndRollToChoiceInternal rc r) | r <- [0..251]] | rc <- [0..461]] 

-- For external access.
indexAndRollToChoice :: RollAndChoiceIndex -> RollIndex -> IntRerollChoice
indexAndRollToChoice rc r = (iaRtCV Vb.! rc) Vu.! r
